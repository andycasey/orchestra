...
cursor.execute("SELECT DISTINCT (drs_blaze_file) FROM obs")
records = cursor.fetchall()
cursor.close()
connection.close()

# Check to see if we have this filename already.
records = ["SAF+{}.tar".format(each[:-13]) \
    for each in blaze_filenames \
    if not os.path.exists(os.path.join(DATA_DIR, each[:-13] + ".tar"))]
...
script_path = os.path.join(DATA_DIR, "download_calibrations.sh")
...
OK now do:
    cd "{}"
    sh download_calibrations.sh

Then untar everything new with:
    grep tar download_calibrations.sh | awk '{{n=split($1,a,"/"); print "tar -xvf \"" a[n] " --keep-old-files --force-local "}}' > untar_calibrations.sh
    sh untar_calibrations.sh

Then fix the folder structure by running:
    cd "{}"
